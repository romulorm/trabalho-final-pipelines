...
def preparar_dados(df, coluna_target='respondeu_campanha'):
    ...
    X = df.drop(columns=[coluna_target, 'cliente_id'])  # Remove a coluna target e cliente_id
...
if __name__ == "__main__":
    ...
    if X is None or y is None:
        print("ERRO: Não foi possível obter X e y.")
    else:
        # Dividir
        X_train, X_test, y_train, y_test = dividir_treino_teste(X, y)
        
        if X_train is None:
            print("ERRO: Não foi possível dividir X em treino e teste.")
        else:
            # Treinar
            modelo = treinar_modelo(X_train, y_train)
            
            if modelo is None:
                print("ERRO: Modelo vazio, favor verificar!")
            else:
